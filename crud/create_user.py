...
def validate_name(name: str, field_name: str, is_required: bool = True) -> bool:
    """
    Проверяет корректность имени/фамилии/отчества согласно требованиям БД
    - Должна быть хотя бы одна буква (не только дефисы/апострофы)
    - Поддерживает: русские буквы, дефисы, пробелы, апострофы (разные типы)
    """
    if not name:
        if is_required:
    ...
    if not re.match(r'^[А-Яа-яЁё\'’"\u2018\u2019\u201B\u2032\u2035\s-]+$', name):
        print(f'Ошибка: поле {field_name} содержит недопустимые символы. '
              f'Допустимы только русские буквы, апострофы, дефисы и пробелы')
        return False
    
    # Проверка что есть хотя бы одна буква (не только дефисы/апострофы)
    ...
    if '  ' in name:
        print(f'Ошибка: поле {field_name} содержит двойные пробелы')
        return False
